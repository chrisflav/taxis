...
private def ActorRow.toActor (r : ActorRow) (db : Conn) : IO Actor := do
  ...
  pure { id := r.id, email := r.email, displayName := r.displayName, groups, googleSub := r.googleSub, admin := r.admin }
...
def getActor (db : Conn) (id : ActorId) : IO (Option Actor) := do
  let rows ← (← db query!"SELECT id, email, display_name, google_sub, admin FROM actors WHERE id = {id}" as ActorRow).toArray
  match rows[0]? with
  | none => pure none
...
def getActorByGoogleSub (db : Conn) (sub : String) : IO (Option Actor) := do
  let rows ← (← db query!"SELECT id, email, display_name, google_sub, admin FROM actors WHERE google_sub = {sub}" as ActorRow).toArray
  match rows[0]? with
  | none => pure none
  | some r => some <$> r.toActor db
...
def getActorByEmail (db : Conn) (email : String) : IO (Option Actor) := do
  let rows ← (← db query!"SELECT id, email, display_name, google_sub, admin FROM actors WHERE email = {email}" as ActorRow).toArray
  match rows[0]? with
  | none => pure none
  | some r => some <$> r.toActor db
...
def listActors (db : Conn) : IO (Array Actor) := do
  let rows ← (← db query!"SELECT id, email, display_name, google_sub, admin FROM actors ORDER BY id" as ActorRow).toArray
...
def createActor (db : Conn) (input : ActorInput) : IO Actor :=
  withTransaction db do
    let rows ← (← db query!"INSERT INTO actors (email, display_name, google_sub, admin)
      VALUES ({input.email}, {input.displayName}, {input.googleSub}, {input.admin})
      RETURNING id, email, display_name, google_sub, admin" as ActorRow).toArray
    let r := rows[0]!
...
def updateActor (db : Conn) (id : ActorId) (upd : ActorUpdate) : IO (Option Actor) :=
  withTransaction db do
    ...
    | some a =>
      ...
      db exec!"UPDATE actors SET email = {email}, display_name = {displayName}, google_sub = {googleSub}, admin = {admin} WHERE id = {id}"
