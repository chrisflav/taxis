...
An actor is someone assigned to or responsible for an issue (a human or a bot). Only actors
with a linked Google account (`googleSub`) can authenticate against the backend. A group is a
collection of actors used as a visibility/permission filter.
-/
...
structure Actor where
  ...
  /-- Google `sub` claim of the linked account, if any. `none` ⇒ cannot authenticate. -/
